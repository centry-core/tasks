...
def write_task_run_logs_to_minio_bucket(project_id, task, task_name, project):
    ...
    if loki_settings_url:
        ...
        if response.ok:
            ...
            file_output.write(f'Task {task_name} (task_result_id={task.task_result_id}) run log:\n'.encode(enc))
